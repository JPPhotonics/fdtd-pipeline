...
res = 15       # simulation resolution, number of cells per wavelength
span = 20     # wavelength span (nm)
um = 0.001    # convert nanometer to micrometer
mode_idx = 2  # Index of the injected mode, Lumerical starts with 1, Tidy3D starts with 0.
...
gds_file_path = os.path.join(
    ...
    'polarization_splitter_rotator.gds'
)
# path to output file
output_dir = os.path.join(
    current_directory, 'projects', 'FDTD_solvers', 'polarization_splitter_rotator', 'Data', solver,
    f'connect_narrow_bw/res{res}_1530nm_span{span}_step5'
)
# task name for Tidy3D
task_name = f'PSR_TM_res{res}_span{span}_step5_{datetime.now().strftime("%Y%m%d%H%M%S")}'

# define the simulation parameter dictionary
p = dict(
    # wavelength span, from nm to um
    wav_span = span*um,                 
