...
res = 20      # simulation resolution, number of cells per wavelength
span = 50     # wavelength span (nm)
step = 0.2    # wavelength step (nm)
...
output_dir = os.path.join(
    current_directory, 'projects', 'FDTD_solvers', 'ring', 'Data', solver,
    f'sweep_resolution/res{res}_span{span}_step5'
)
# task name for Tidy3D
task_name = f'RING_res{res}_span{span}_step5_{datetime.now().strftime("%Y%m%d%H%M%S")}'

# define the simulation parameter dictionary
p = dict(
    # wavelength span in micrometer
    wav_span = span * um,
    # wavelength step in micrometer,
    wav_step = step * um,
